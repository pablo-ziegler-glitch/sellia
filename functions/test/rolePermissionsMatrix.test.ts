...
describe("rolePermissionsMatrix critical regression", () => {
  ...
  it("limits critical modules to owner only", () => {
    for (const module of CRITICAL_MODULES) {
      expect(MODULE_ROLE_POLICIES[module]).toEqual(["owner"]);
      expect(hasRoleForModule("owner", module)).toBe(true);
      expect(hasRoleForModule("admin", module)).toBe(false);
  ...
  it("normalizes role input before authorization", () => {
    ...
    expect(hasRoleForModule("ADMIN", "usersRoles")).toBe(false);
